...
export const EstadisticaPartidoSenado = ({ route }) => {
  ...
  const cargarDetalleMociones = async () => {
    ...
    try {
      ...
      const data =
        await partidosRepository.getDetalleMocionesPartidoSenadores(partidoId);

      const agrupadas = Object.values(
        data.reduce((acc, row) => {
          const boletin = row.numeroBoletin;

          if (!acc[boletin]) {
            acc[boletin] = {
              numeroBoletin: boletin,
              titulo: row.tituloMocion || "",
              votaciones: [],
            };
          }
      ...
      const procesadas = agrupadas.map((mocion) => {
        ...
        return {
          ...
          votaciones: votacionesOrdenadas.map((votacion) => {
            const temaActual = votacion.tema || "";
            const temaNormalizado = normalizarTexto(temaActual);

            const mostrarTema =
              temaNormalizado !== "" &&
              temaNormalizado !== temaAnterior;

            temaAnterior = temaNormalizado;

            return {
              ...votacion,
              temaMostrar: mostrarTema ? temaActual : null,
            };
          }),
        };
      });
      ...
      console.error("Error cargando detalle de mociones Senado:", error);
    } finally {
  ...
  const getMetricasHistoricas = async () => {
    try {
      const data =
        await partidosRepository.getMetricasHistoricasPartidoSenadores(partidoId);
  ...
  const getCompatibilidadPartido = async () => {
    try {
      ...
      const data =
        await partidosRepository.getCompatibilidadUsuarioPartidoSenadores(
          user.id,
          partidoId,
        );
  ...
  const getMocionesAprobadas = async () => {
    try {
      const data =
        await partidosRepository.getMocionesAprobadasPartidoSenadores(
          partidoId,
        );
  ...
  const getEstadisticasGenerales = async () => {
    try {
      const data =
        await partidosRepository.getEstadisticasGeneralesPartidoSenadores(partidoId);
  ...
  const getCohesionPartido = async () => {
    try {
      ...
      const data =
        await partidosRepository.getCohesionPartidoSenadores(partidoId);
  ...
  const obtenerLegisladoresPorPartido = async () => {
    try {
      const data =
        await partidosRepository.getEstadisticasSenadoresPartido(partidoId);
      ...
      console.error(
        "Error cargando senadores y estadísticas del partido:",
        error,
      );
  ...
  const detalleMocionesFiltradas = detalleMociones.filter((mocion) => {
    ...
    const contenidoMocion = [
      ...
      ...(mocion.votaciones ?? []).flatMap((votacion) => [
        votacion.resultado,
        votacion.tema,
        votacion.temaMostrar,
        votacion.numeroSesion,
        votacion.fechaTexto,
        votacion.quorum,
      ]),
    ]
                    ...
                    centerLabelComponent={() => (
                      <Text
                        style={{
                          color: COLORS.greenM,
                          fontSize:
                            String(mocionesAprobadas.fraccion).length > 4
                              ? Math.max(11, responsiveWidthScale(11.5))
                              : Math.max(11, responsiveWidthScale(14)),
              ...
              <View style={styles.columnaEstadistica}>
                ...
                >
                  <View style={styles.container5}>
                    <View style={styles.circulo}>
                      <Text style={styles.data2}>
                        {partido.rankingEstadisticoSenado
                          ? `${partido.rankingEstadisticoSenado}`
                          : "-"}
                      </Text>
                    </View>
