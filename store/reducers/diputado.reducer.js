...
const DiputadoReducer = (state = initialState, action) => {
  switch (action.type) {
    case FILTERED_DIPUTADOS:
      return {
        ...state,
        filteredDiputados: state.diputados.filter(
          (diputadosFilter) => diputadosFilter.distrito === action.distritoID
        ),
    ...
    case SELECT_DIPUTADO:
      return {
        ...state,
        selected: state.diputados.find(
          (diputado) => diputado.id === action.diputadoID
        ),
      };
